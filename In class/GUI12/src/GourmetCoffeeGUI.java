...
public class GourmetCoffeeGUI extends JPanel {
    ...
    private JPanel getDataFieldsPanel(ArrayList<DataField> dataFields) {
        ...
        GridLayout g1 = new GridLayout(9, 2, 5, 5);

        //读取信息
        for (DataField dataField : dataFields) {
            JLabel fieldNameLabel = new JLabel(dataField.getName(), JLabel.HORIZONTAL);
            JTextField fieldValueField = new JTextField(dataField.getValue(), 18);
            //设置为不可编辑
            fieldValueField.setEditable(false);

            panel.add(fieldNameLabel);
            panel.add(fieldValueField);
        }
...
//        panel.setVisible(true);
    ...
    class AddModifyListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            //非法输入
            if (quantity < 0) {
                statusTextArea.setText("Illegal input");
                return;
            }
            boolean flag = false;
            ...
            for (OrderItem orderItem : currentOrder.getItems()) {
                //如果有，就把他的数量设置成textfield中用户输入的
                if (orderItem.getProduct().getCode().equals(code)) {
    ...
    class RemoveListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent event) {
            if(orderList.isSelectionEmpty()){
                statusTextArea.setText("未做选择");
            }
            if(currentOrder.getNumberOfItems() == 0){
                statusTextArea.setText("订单表为空");
            }
    ...
    class SaveSalesListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent event) {
            ...
            if (returnVal == JFileChooser.APPROVE_OPTION) {
                ...
                //从catalog.dat读取销售信息
                RandomAccessFile raf = new RandomAccessFile("catalog.dat","r");

                // 写入文件
                try (BufferedWriter writer = new BufferedWriter(new FileWriter(selectedFile))) {

                    displayStatusMessage("Sales information saved successfully.");
                } catch (IOException e) {
                    // 捕获异常并显示错误消息
                    if (e.getMessage().contains("No such file or directory")) {
                        displayErrorMessage("catalog.dat file not found.");
                    } else if (e.getMessage().contains("Cannot create directory")) {
                        displayErrorMessage("The directory where the file is to be created does not exist.");
                    } else {
                        displayErrorMessage("An error occurred while reading or writing the file.");
                    }
                    e.printStackTrace();
                }
            } else {
        ...
        private void displayErrorMessage(String message) {
            statusTextArea.append("Error: " + message + "\n");
